...
def DisplayLogin(WindowMain,frameLogin,frameMain,framePoint1,framePoint2,framePoint3,framePoint4,framePoint5):
    ...
    Label(frameLogin, text="Nom d'utilisateur", font=("Impact",15),foreground="white", background="#009790").grid(column=2, row=2,padx=250)
    Entry(frameLogin, textvariable=usernamevalue).grid(column=2, row=3, pady=15, padx=15)
    
    Label(frameLogin, text="Mot de passe", font=("Impact",15),foreground="white", background="#009790").grid(column=2, row=4)
    Entry(frameLogin, textvariable=passwordvalue,show="*").grid(column=2, row=5, pady=15,padx=15)
    
    Button(frameLogin, text="Connexion", command=lambda:connexion(usernamevalue.get(),passwordvalue.get(),WindowMain,frameMain,frameLogin,framePoint1,framePoint2,framePoint3,framePoint4,framePoint5)).grid(column=2, row=9, pady=30, ipadx=50)
